...
function uvPyramid(sides, radius) {
   sides = sides || 4;
   radius = radius || 1;
   const height = radius * Math.sqrt(2);

   const vertexCount = sides + 1 + sides + 1;
   const vertices = [];
   const normals = [];
   const texCoords = [];
   const indices = [];
   ...
   const baseCenterIndex = sides;

   for (let i = 0; i < sides; i++) {
       indices.push(baseCenterIndex, i, (i + 1) % sides);
   }
   ...
   for (let i = 0; i < sides; i++) {
       ...
       indices.push(apexIndex, (i + 1) % sides, i);
   }
